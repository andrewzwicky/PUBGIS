...
def test_bad_images(test_image, pubgis_fixture):
    match_found, coords, _ = pubgis_fixture.template_match_minimap((None, cv2.imread(os.path.join('bad', test_image))))
...
def test_good_images(test_image, pubgis_fixture):
    match_found, coords, _ = pubgis_fixture.template_match_minimap((None, cv2.imread(os.path.join('good', test_image))))
    assert match_found
