...
def create_output(input_map, coords, output_file, color=PATH_COLOR, thickness=PATH_THICKNESS):
    ...
    output_axis.imshow(cv2.cvtColor(input_map, cv2.COLOR_BGR2RGB))

    (min_x, min_y), size = find_path_bounds(input_map.shape[0], filtered_coords)
    ...
    output_axis.plot(*zip(*filtered_coords), color=mpl_color, linewidth=thickness)
